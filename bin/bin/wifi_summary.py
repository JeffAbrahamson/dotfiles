...
def get_data(ssid):
    ...
    df_ping = pd.read_table(
        data_dir + "ping", sep=" ", names=["timestamp", "ping"]
    )
    df_download = pd.read_table(
        data_dir + "download", sep=" ", names=["timestamp", "download"]
    )
    df_upload = pd.read_table(
        data_dir + "upload", sep=" ", names=["timestamp", "upload"]
    )
    df_ssid = pd.read_table(
        data_dir + "ssid", sep=" ", names=["timestamp", "ssid"]
    )

    df = df_ping.set_index("timestamp").join(
        df_download.set_index("timestamp"), how="inner"
    )
    ...
    title = (
        f"{ssid} wifi performance "
        "(point size is ping time, larger is worse)"
    )
    plt.title(title)
