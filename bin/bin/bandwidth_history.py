...
def plot_ping_latency_by_day(dframe):
    ...
    ping_latency_df = dframe.loc[
        :, ["-ping" in x and "speedtest" not in x for x in dframe.columns]
    ].copy()
    ...
    plt.scatter(
        mean_latency_df.date, mean_latency_df.clipped_latency, s=1, c="orange"
    )
    ...
    plt.scatter(
        dframe_p_lower.index,
        dframe_p_lower.ewm(alpha=alpha).mean(),
        s=1,
        c="darkblue",
    )
    plt.scatter(
        dframe_p_upper.index,
        dframe_p_upper.ewm(alpha=alpha).mean(),
        s=2,
        c="slateblue",
    )
    ...
    plt.xlabel(
        "EWMA of "
        + str(lower_percentile)
        + " and "
        + str(upper_percentile)
        + " percentiles with alpha = "
        + str(alpha)
    )
    plt.ylim((0, None))
    plt.show()
...
def plot_ping_uptime_by_day(dframe):
    """Plot."""
    ping_uptime_df = dframe.loc[
        :, ["-up" in x and "speedtest" not in x for x in dframe.columns]
    ].copy()
